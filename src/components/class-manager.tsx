...
export function ClassManager({ classes }: { classes: ClassVM[] }) {
  ...
  function submit(e: FormEvent) {
    ...
    startTransition(async () => {
      apply(
        draft.id ? { kind: "update", item: vm } : { kind: "add", item: vm },
      );
      const res = draft.id
        ? await updateClassAction(fd)
    ...
    <div className="rise">
      <div className="mb-4 flex items-center justify-between">
        <p className="text-[0.85rem] text-ink-soft">
          <strong className="text-ink">{list.length}</strong> class
          {list.length === 1 ? "" : "es"} on your load
        </p>
        <button
          type="button"
          ...
          {list.map((c, i) => (
            ...
            >
              ...
              <div className="flex items-start justify-between gap-2">
                ...
                <span className={`chip !border-0 ${toneClass[c.healthTone]}`}>
                  {c.healthLabel}
                </span>
              </div>
              ...
              <div className="mt-4 grid grid-cols-3 gap-2 text-center">
                {[
                  ...
                  {
                    n:
                      c.studentCount && c.lessonsPerWeek
                        ? Math.ceil(c.studentCount / 5)
                        : "—",
                    l: "≈ /day pace",
                  },
                ].map((s) => (
                  <div key={s.l} className="rounded-xl bg-cream/70 px-2 py-2.5">
                    <p className="font-display text-[1.05rem] font-semibold leading-none text-ink">
                      {s.n}
                    </p>
                    <p className="mt-1 text-[0.62rem] font-semibold uppercase tracking-wide text-ink-faint">
                      {s.l}
                    </p>
                  </div>
                ))}
        ...
        {draft ? (
          <form onSubmit={submit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="col-span-2 sm:col-span-1">
                <label className="label" htmlFor="cf-year">
                  Year group
                </label>
                <select
                  id="cf-year"
                  ...
                  onChange={(e) =>
                    setDraft({ ...draft, yearGroup: e.target.value })
                  }
                >
                  {YEARS.map((y) => (
              ...
              <div className="col-span-2 sm:col-span-1">
                <label className="label" htmlFor="cf-count">
                  Students
                </label>
                <input
                  id="cf-count"
                  ...
                  onChange={(e) =>
                    setDraft({ ...draft, studentCount: e.target.value })
                  }
                  required
                />
              </div>
                ...
                {COLORS.map((col) => (
                  <button
                    ...
                    style={{
                      background: col,
                      transform:
                        draft.color === col ? "scale(1.15)" : undefined,
                      boxShadow:
                        draft.color === col ? `0 0 0 3px ${col}44` : undefined,
                    }}
