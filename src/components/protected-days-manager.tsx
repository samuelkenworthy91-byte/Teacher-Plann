  ...
  return (
    <section className="card rise p-6" style={{ animationDelay: "180ms" }}>
      <div className="flex items-start gap-3">
        ...
        <div>
          <h2 className="font-display text-lg font-semibold text-ink">Protected days</h2>
          <p className="mt-0.5 text-[0.78rem] leading-relaxed text-ink-soft">
            Block holidays, days you do not work, or days you will not mark. The planner skips them
            for collections and hand-backs, and they do not count toward a marking target.
          </p>
        </div>
      </div>
      ...
      >
        <div>
          <label className="label" htmlFor="protected-from">From</label>
          <input
            id="protected-from"
            type="date"
            className="input"
        ...
        <div>
          <label className="label" htmlFor="protected-to">To</label>
          <input
            id="protected-to"
            type="date"
            className="input"
      ...
      <div className="mt-5 border-t border-line pt-4">
        <div className="mb-2 flex items-center justify-between gap-3">
          ...
          <p className="text-[0.7rem] text-ink-faint">Weekends are always skipped</p>
        </div>
        {upcoming.length === 0 ? (
          <p className="rounded-xl border border-dashed border-line-strong px-4 py-4 text-center text-[0.78rem] text-ink-faint">
            No protected weekdays yet. Add a day or date range above when your routine changes.
          </p>
        ) : (
          <ul className="max-h-56 space-y-1.5 overflow-y-auto pr-1">
            {upcoming.map((day) => (
              ...
              >
                <span className="font-semibold text-ink">{pretty(day.date)}</span>
        ...
        {past > 0 ? (
          <p className="mt-2 text-[0.7rem] text-ink-faint">
            {past} past protected day{past === 1 ? "" : "s"} kept in your record.
          </p>
        ) : null}
      </div>
    </section>
