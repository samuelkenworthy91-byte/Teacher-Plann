...
import { PlanAutoRefresh } from "@/components/plan-auto-refresh";
        ...
        return (
          ...
          >
            <Icon
              size={17}
              strokeWidth={2.2}
              className={active ? "text-pen" : ""}
              style={active ? { color: "#f0916f" } : undefined}
            />
        ...
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <aside className="pop absolute inset-y-0 left-0 flex w-[272px] flex-col bg-dark py-6 shadow-2xl">
            <div className="flex items-center justify-between px-7 pb-7">
              <Logo dark />
              <button
                className="rounded-lg p-1.5 text-cream/60 hover:bg-cream/10"
                onClick={() => setDrawer(false)}
              >
                <X size={18} />
              </button>
        ...
        <header className="sticky top-0 z-20 border-b border-line bg-paper/85 backdrop-blur-md">
          <div className="mx-auto flex h-16 max-w-6xl items-center gap-3 px-4 sm:px-6">
            ...
            <div className="flex flex-1 items-center gap-3">
              ...
              <PlanAutoRefresh needed={needsPlanRefresh} />
            ...
            <div className="relative">
              ...
              >
                ...
                {urgent > 0 ? (
                  <span
                    className="absolute -right-0.5 -top-0.5 grid h-4.5 min-w-4.5 place-items-center rounded-full bg-pen px-1 text-[0.6rem] font-bold text-white"
                  ...
                  <div className="pop card absolute right-0 z-50 mt-2 w-[min(92vw,380px)] overflow-hidden p-0">
                    ...
                    <ul className="max-h-[60vh] overflow-y-auto">
                      {notices.map((n) => (
                        <li
                          key={n.id}
                          className="flex gap-3 border-b border-line px-4 py-3.5 last:border-0"
                        >
                          <Dot color={TONE_COLOR[n.tone]} size={9} />
                          <div className="min-w-0">
                            <p className="text-[0.83rem] font-semibold leading-snug text-ink">
                              {n.title}
                            </p>
