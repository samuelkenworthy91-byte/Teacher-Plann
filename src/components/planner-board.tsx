...
export function PlannerBoard({
  ...
  const [list, applyDelete] = useOptimistic<PlanVM[], number>(
    plans,
      ...
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-[0.78rem] text-ink-soft">
            Confirm real collection and hand-back dates as they happen. Edited
            entries stay fixed; every unlocked future task reflows around them.
          </p>
        </div>
          ...
          {groups.map(([week, items]) => (
            <div key={week}>
              <p className="mb-2 text-[0.7rem] font-bold uppercase tracking-[0.14em] text-ink-faint">
                Week of{" "}
                {items[0]?.collectLabel
                  ? new Date(week + "T00:00:00Z").toUTCString().slice(0, 16)
                  : week}
              </p>
              <div className="space-y-2.5">
                {items.map((p) => (
                  ...
                  >
                    <span
                      className="hidden h-11 w-1.5 rounded-full sm:block"
                    ...
                    <div className="min-w-[120px] flex-1">
                      <div className="flex items-center gap-2">
                        ...
                        {p.planType === "manual" ? (
                          <span className="chip !py-1 !text-[0.62rem]">
                            manual
                          </span>
                        ) : null}
                        {p.late ? (
                    ...
                    <div className="flex items-center gap-2 text-[0.8rem] font-semibold text-ink">
                      <span className="flex items-center gap-1.5 rounded-lg bg-pen-soft px-2.5 py-1.5 text-pen">
                        ...
                        {p.collectLabel}
                        {p.collectPeriod ? (
                          <span className="opacity-70">P{p.collectPeriod}</span>
                        ) : null}
                      </span>
                    ...
                    <span className="chip !border-0 !bg-ink !py-1.5 !pl-2.5 !text-cream">
                      mark ≥ <strong className="px-0.5">{p.dailyRate}</strong>
                      /day
                    </span>
                    ...
                    <div className="ml-auto flex items-center gap-1">
                      ...
                      <button
                        ...
                        title={
                          p.locked
                            ? "Unlock (allow regeneration to move it)"
                            : "Lock (keep these dates)"
                        }
                        onClick={() =>
                          startTransition(
                            async () => void (await toggleLockAction(p.id)),
                          )
                        }
                      >
                        {p.locked ? (
                          <Lock size={14} className="text-pen" />
                        ) : (
                          <LockOpen size={14} />
                        )}
                      </button>
      ...
      >
        {editing ? (
          <form onSubmit={saveEdit} className="space-y-4">
            ...
            <div>
              <label className="label" htmlFor="ep-title">
                Task
              </label>
              <input
                id="ep-title"
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="label" htmlFor="ep-collect">
                  Collect
                </label>
                <input
                  id="ep-collect"
              ...
              <div>
                <label className="label" htmlFor="ep-hand">
                  Hand back
                </label>
                <input
                  id="ep-hand"
            ...
            <div>
              <label className="label" htmlFor="ep-total">
                Books in the pile
              </label>
              <input
                id="ep-total"
