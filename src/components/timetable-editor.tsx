...
export function TimetableEditor({
  ...
  const [grid, apply] = useOptimistic<
    SlotVM[],
    { day: number; period: number; classId: number | null }
  >(slots, (state, op) => {
    const rest = state.filter(
      (s) => !(s.dayOfWeek === op.day && s.period === op.period),
    );
    return op.classId === null
      ? rest
      : [
          ...rest,
          { dayOfWeek: op.day, period: op.period, classId: op.classId },
        ];
  });
      ...
      <div className="overflow-x-auto">
        ...
        >
          ...
          <tbody>
            {periods.map((p) => (
              <tr key={p}>
                <td className="pr-1 text-center text-[0.68rem] font-bold text-ink-faint">
                  P{p}
                </td>
                {DAYS.map((_, di) => {
                  const day = di + 1;
                  ...
                  return (
                    <td key={day}>
                      <button
                        type="button"
                        onClick={() =>
                          setSelected(isSel ? null : { day, period: p })
                        }
        ...
        <div className="pop mt-4 rounded-xl border border-line bg-white p-3.5">
          <div className="mb-2.5 flex items-center justify-between">
            <p className="text-[0.72rem] font-bold uppercase tracking-[0.12em] text-ink-soft">
              {DAYS[selected.day - 1]} · Period {selected.period} — who&apos;s
              here?
            </p>
            <button
              type="button"
          ...
          <div className="flex flex-wrap gap-2">
            {classes.map((c) => {
              const current =
                at(selected.day, selected.period)?.classId === c.id;
              return (
                <button
                  key={c.id}
                  type="button"
            ...
            {pending ? (
              <Spinner className="self-center text-ink-faint" />
            ) : null}
          </div>
        </div>
      ) : (
        <p className="mt-3 text-center text-[0.72rem] text-ink-faint">
          Tap any cell to assign a class. This grid is the input for every
          scheduling decision.
        </p>
      )}
    </div>
      ...
      <div>
        ...
        >
          ...
          <span className="text-[0.8rem] font-semibold text-ink">
            Snap or upload the photo
          </span>
          <span className="max-w-[220px] text-[0.7rem] leading-relaxed text-ink-faint">
            One clear shot of your paper timetable. It never leaves your
            account.
          </span>
        </button>
        {error ? (
          <p className="mt-2 text-[0.74rem] font-medium text-bad">{error}</p>
        ) : null}
      </div>
    );
  }
