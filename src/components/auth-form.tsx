  ...
  return (
    <div className="mt-8">
      ...
      <form action={formAction} className="space-y-4">
        ...
        <button
          type="submit"
          className="btn btn-ink w-full !py-3 !text-[0.85rem]"
          disabled={pending}
        >
      ...
      {mode === "login" ? (
        <>
          <div className="my-6 flex items-center gap-3 text-xs font-semibold uppercase tracking-widest text-ink-faint">
            <hr className="divider flex-1" /> or{" "}
            <hr className="divider flex-1" />
          </div>
          <form action={demoAction}>
            <button
              type="submit"
              className="btn btn-ghost w-full !py-3 !text-[0.85rem]"
          ...
          <p className="mt-3 text-center text-xs leading-relaxed text-ink-faint">
            A fully-seeded science department: live marking, a class due today,
            and the next fortnight already planned.
          </p>
        </>
      ) : null}
    </div>
