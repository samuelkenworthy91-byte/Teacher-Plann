...
export function lessonsFor(
  ...
  for (let d = toDay(from); d <= toDay(to); d++) {
    ...
    if (w < 1 || w > 5 || !isAvailableSchoolDay(date, unavailableDates))
      continue;
    for (const s of mine) {
...
export function requiredToday(
  ...
  // A protected day never produces a marking target.
  if (remaining === 0 || !isAvailableSchoolDay(today, unavailableDates))
    return 0;
...
export function generateSchedule(args: {
  ...
  const jobs: Job[] = classes
    ...
    .map((cls) => {
      ...
      const latestByLessons =
        future[settings.maxLessons - 1]?.date ??
        addDays(last, settings.maxGapDays);
      const latest = minDate(
        addDays(last, settings.maxGapDays),
        latestByLessons,
      );
    ...
    .sort(
      (a, b) => cmp(a.latest, b.latest) || a.cls.name.localeCompare(b.cls.name),
    );
  ...
  for (const job of jobs) {
    ...
    if (candidates.length === 0) {
      const next = firstLessonOnOrAfter(
        cls.id,
        slots,
        today,
        45,
...
export function computeClassHealth(
  ...
  if (
    nextPlanned &&
    (nextPlanned.status === "marking" ||
      cmp(nextPlanned.collectDate, today) <= 3)
  ) {
    status = "fresh"; // handled — a cycle is booked in
  } else if (days >= settings.maxGapDays || lessons >= settings.maxLessons)
    status = "overdue";
  else if (days >= settings.maxGapDays - 4 || lessons >= settings.minLessons)
    status = "due";
