...
export default async function DashboardPage() {
  ...
  if (classes.length === 0) {
    return (
      <div className="rise mx-auto max-w-2xl pt-6">
        <p className="text-[0.72rem] font-bold uppercase tracking-[0.16em] text-pen">
          Getting started
        </p>
        ...
        <p className="mt-3 text-[0.95rem] leading-relaxed text-ink-soft">
          Three quick steps and MarkFlow starts telling you what to collect, how
          much to mark each day, and when to hand it back.
        </p>
          ...
          {[
            {
              ...
              body: "Names, year groups and class sizes.",
              href: "/classes",
              cta: "Add a class",
            },
            {
              n: "2",
              icon: CalendarPlus,
              title: "Snap & tap your timetable",
              body: "Upload the photo once, then tap the grid to match it.",
              href: "/timetable",
              cta: "Open timetable",
            },
            {
              ...
              body: "We schedule every formative so piles never collide.",
              href: "/planner",
              cta: "Open planner",
            },
          ].map((s) => (
            ...
            >
              ...
              <p className="mt-3 font-display text-[1.05rem] font-semibold text-ink">
                {s.title}
              </p>
              <p className="mt-1 text-[0.8rem] leading-relaxed text-ink-soft">
                {s.body}
              </p>
              <p className="mt-3 text-[0.78rem] font-bold text-pen">
                {s.cta} →
              </p>
            </Link>
          ))}
  ...
  const nextHandback = [...livePlans].sort((a, b) =>
  ...
  const weekEvents = weekDays.map((d) => ({
    ...
    events: livePlans
      ...
      .map((p) => ({
        type:
          p.collectDate === d ? ("collect" as const) : ("handback" as const),
  ...
  const STAT_CARDS = [
    ...
    },
    {
      ...
      sub: nextHandback
        ? (classById.get(nextHandback.classId)?.name ?? "")
        : "nothing booked",
    },
    {
      icon: CheckCheck,
    ...
    <div className="space-y-8">
          ...
          <div className="card flex flex-wrap items-center gap-5 p-6">
            ...
            <div className="min-w-0 flex-1">
              <p className="font-display text-lg font-semibold text-ink">
                No pile on your desk
              </p>
              <p className="text-sm text-ink-soft">
                Nothing collected right now. Your next collection appears here
                the moment it&apos;s due.
              </p>
            </div>
          </div>
      ...
      <section className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        {STAT_CARDS.map((s, i) => (
          <div
            key={s.label}
            className="card card-hover rise p-4.5 p-5"
            style={styleDelay(2 + i)}
          >
            <div className="flex items-center justify-between">
              <p className="text-[0.68rem] font-bold uppercase tracking-[0.12em] text-ink-faint">
                {s.label}
              </p>
            ...
            {weekEvents.map((d, di) => (
              <div
                ...
                className={`min-h-[110px] rounded-xl border p-2 ${
                  d.isToday
                    ? "border-pen/50 bg-pen-soft/50"
                    : "border-line bg-white/60"
                }`}
              >
                <p
                  className={`text-[0.68rem] font-bold uppercase tracking-wide ${d.isToday ? "text-pen" : "text-ink-faint"}`}
                >
                  {WEEKDAY_SHORT[di]}
                </p>
                <p
                  className={`font-display text-[0.95rem] font-semibold ${d.isToday ? "text-pen" : "text-ink"}`}
                >
                  ...
                  {d.events.map((ev, i) => (
                    ...
                    >
                      ...
                      {ev.type === "collect" ? (
                        <ArrowDownToLine
                          size={9}
                          className="ml-auto shrink-0 text-pen"
                        />
                      ) : (
                        <ArrowUpFromLine
                          size={9}
                          className="ml-auto shrink-0 text-good"
                        />
                      )}
                    </div>
                  ))}
        ...
        <div className="mb-4 flex items-end justify-between">
          <div>
            ...
            <p className="text-[0.8rem] text-ink-soft">
              Every class, measured against your {settings.minLessons}–
              {settings.maxLessons} lesson / {settings.maxGapDays}-day rules.
            </p>
          </div>
            ...
            return (
              <div key={h.classId} className="card card-hover p-4">
                ...
                <div className="mt-3 flex items-center justify-between text-[0.72rem] text-ink-soft">
                  <span>
                    <strong className="text-ink">{h.lessonsSince}</strong>{" "}
                    lessons ·{" "}
                    <strong className="text-ink">{h.daysSince}</strong>d since
                    feedback
                  </span>
                </div>
                {h.nextPlanned ? (
                  ...
                  <p className="mt-1.5 text-[0.72rem] font-medium text-warn">
                    No cycle booked — run the planner
                  </p>
                )}
              </div>
            );
