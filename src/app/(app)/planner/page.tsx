    ...
    <div className="space-y-6">
      <header className="rise">
        ...
        <p className="mt-1 max-w-2xl text-[0.88rem] leading-relaxed text-ink-soft">
          ...
          , never more than{" "}
          <strong className="text-ink">{settings.maxGapDays} days</strong>{" "}
          between hand-backs, and windows are staggered so{" "}
          <strong className="text-ink">only one pile</strong> is ever on your
          desk. Missed, unlocked collections roll forward automatically.
        </p>
      </header>
      ...
      {clashes.length === 0 ? (
        <div
          className="rise flex items-center gap-3 rounded-2xl border border-good/25 bg-good-soft px-5 py-4"
          style={{ animationDelay: "40ms" }}
        >
          <ShieldCheck size={20} className="shrink-0 text-good" />
          <p className="text-[0.85rem] font-medium text-good">
            No clashes in the diary — you&apos;ll never be marking two classes
            at once.
          </p>
        </div>
      ) : (
        <div
          className="rise rounded-2xl border border-warn/30 bg-warn-soft px-5 py-4"
          style={{ animationDelay: "40ms" }}
        >
          ...
          <p className="mt-1 text-[0.78rem] text-warn">
            Two piles at once — edit dates below, or regenerate to re-stagger
            the diary.
          </p>
        </div>
      )}
      ...
      <section className="card rise p-5" style={{ animationDelay: "80ms" }}>
        <div className="mb-4 flex items-center justify-between">
          ...
          <p className="text-[0.72rem] font-semibold text-ink-faint">
            expected books / day
          </p>
        </div>
            ...
            >
              ...
              <div className="flex h-24 w-full items-end overflow-hidden rounded-lg bg-ink/5">
                <div
                  ...
                  style={{
                    height: `${Math.max(2, (w.load / maxLoad) * 100)}%`,
                  }}
                  title={`${w.load} books/day across ${w.plans} pile(s)`}
                />
              </div>
              <p
                className={`text-[0.62rem] font-bold uppercase tracking-wide ${w.start === monday ? "text-pen" : "text-ink-faint"}`}
              >
