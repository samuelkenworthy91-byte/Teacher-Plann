      ...
      <header className="rise flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 max-w-xl text-[0.88rem] text-ink-soft">
            One pile at a time. Log books as you go — the daily minimum always
            reflects what&apos;s genuinely left.
          </p>
        </div>
        <TaskCreator
            ...
            <ul className="space-y-2.5">
              {history.map((p) => {
                ...
                return (
                  <li
                    key={p.id}
                    className="flex flex-wrap items-center gap-3 rounded-xl border border-line bg-white/70 px-3.5 py-3"
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-[0.72rem] text-ink-soft">
                        {p.title} · {p.totalBooks} books · back{" "}
                        {p.returnedAt ? prettyShort(p.returnedAt) : "—"}
                      </p>
                    </div>
