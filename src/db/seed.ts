...
async function main() {
  ...
  const slotRows = await db
    ...
    .returning();
  console.log(
    `  · ${slotRows.length} timetable slots across ${PERIODS} periods`,
  );
  ...
  const mkReturned = async (
    ...
    for (const count of pace) {
      await db
        ...
        .values({ planId: plan.id, userId: demo.id, date: dayCursor, count });
  ...
  await db.insert(markingEntries).values([
    { planId: activePlan.id, userId: demo.id, date: collect10A, count: 6 },
    {
      ...
      userId: demo.id,
      date: addSchoolDays(collect10A, 1),
      count: 6,
    },
  ]);
  ...
  const eightXHandback = computeHandback(
    eightX.id,
    slotRows,
    { date: today, period: eightXPeriodToday },
    5,
  );
  ...
  const settings = (
    await db.select().from(userSettings).where(eq(userSettings.userId, demo.id))
  )[0];
  ...
  if (suggestions.length > 0) {
    await db.insert(markingPlans).values(
      suggestions.map((s) => ({
        ...
        totalBooks:
          inserted.find((c) => c.id === s.classId)?.studentCount ?? 30,
  ...
  console.log(
    `  · ${2 + 1 + 1 + suggestions.length} marking cycles (returned, active, due today, upcoming)`,
  );
